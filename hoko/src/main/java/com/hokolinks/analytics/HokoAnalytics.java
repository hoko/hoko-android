...
import com.hokolinks.model.HokoSession;
import com.hokolinks.model.HokoUser;
import com.hokolinks.model.exceptions.HokoIgnoringKeyEventException;
import com.hokolinks.utils.lifecycle.HokoApplicationLifecycle;
import com.hokolinks.utils.lifecycle.HokoApplicationLifecycleCallback;
...
public class HokoAnalytics implements HokoHandler {
     ...
     *HokoAnalytics.HokoUserAccountType.DEFAULT);
     * }</pre>
     *
     ...
     * in the Hoko service.
     * <pre>{@code
     * Hoko.analytics().identifyUser("john.doe", HokoAnalytics.HokoUserAccountType.GITHUB,
     * "John Doe", "john.doe@email.com", new Date(), HokoAnalytics.HokoUserGender.MALE);
     * }</pre>
     *
     * @param identifier  A unique identifier for the user in the scope of your application.
     * @param accountType The account type in which the user fits.
     ...
     */
    public void identifyUser(String identifier, HokoUserAccountType accountType, String name,
                             String email, Date birthDate, HokoUserGender gender) {
        if (mUser != null || !identifier.equalsIgnoreCase(mUser.getIdentifier())) {
            ...
            mUser = new HokoUser(identifier, accountType, null, email, birthDate, gender,
                    previousIdentifier);
            mUser.save(mContext);
            mUser.post(mToken, mContext);
    ...
    /**
     * HokoHandler Interface implementation, receives a handle(deeplink) call from the
     * HokoDeeplinking module. This will render the previous session as "ended" and will start a new
